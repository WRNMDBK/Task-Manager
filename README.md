...
一个基于 Spring Boot、MyBatis 和 MySQL 的任务管理后端练习项目，用于记录待办事项、查询任务进度、完成任务及保存操作记录。

项目重点是练习后端分层、MyBatis 数据访问和 Spring 事务：完成任务时，更新任务状态与写入完成记录应在同一个事务中提交，发生异常时一起回滚。
...
本 README 参考项目设计书编写，接口行为和依赖版本以当前源码为准。
